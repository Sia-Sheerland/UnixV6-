...
public:
	...
	/* 各节在文件中的原始偏移和虚拟大小，供缺页处理程序按需加载使用 */
	...
	unsigned long SectionVirtSize[MAX_SECTIONS];     /* VirtualSize（内存中大小） */
	unsigned long SectionRawSize[MAX_SECTIONS];      /* SizeOfRawData（文件中大小） */
	unsigned int  NumSections;                       /* 实际节数 */
